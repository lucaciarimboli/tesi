...
def Newton_varf(mesh, x, G, phi, psi, psi_N, psi_old, psi_denom, plasma_mask, j_cv, j_coils, vacuum_tag, vessel_tag, coils_tags):
    """
    ...
    dG_dpsi = - diff( G(x,psi_N), psi_N) #/ psi_denom
    '''
    r0 = 6.2
    alpha = 2.0
    beta = 0.5978
    ...
    # Define the bilinear form:
    a = ( 1 / (mu0 * x) * dot(grad(psi), grad(phi)) ) * dx(domain=mesh) \
        - plasma_mask * dG_dpsi * psi * phi * dx(vacuum_tag, domain=mesh)

    # Add the contribution from the r.h.s
    a += plasma_mask * dG_dpsi * psi_old * phi * dx(vacuum_tag, domain=mesh) \
        - plasma_mask *  G(x,psi_N) * phi * dx(vacuum_tag, domain=mesh) \
        - j_cv * phi * dx(vessel_tag, domain=mesh)
