...
class JN_coupling_BCs:
    ...
    @TODO: Account for the PF coils in the BCs so that the problem domain can be smaller
        and include the vacuum chamber only.

    (!) Currently, the domain includes the symmetry axis of the tokamak (R=0).
        This causes problems to the calculation of the Neumann datum q on the boundary.   
        For this reason, this class is not used currently in the main solver and 
        homogeneous Dirichlet BCs are imposed instead, forcing to have a domain significantly
        larger than the tokamak structure.
    '''
    ...
    def dofs_indexes(self, V):
        ...
        @param V: Function space on the 2D mesh.
        @return: array containing the idexes of the dofs where Green function is evaluated.
        '''
    ...
    def assemble_matrix(self,V):
        '''
            ...
            Integrals close to singularities are handled with asymptotic expansions, while the ones
            far from singularities are computed with simple trapezoidal quadrature.
            
            @TODO: Test if the handling of singularities is correct.
        '''
        mu0 = 4e-7 * pi
        ...
        # extract all indexes except the extremes:
        idxs = [i for i in range(n+2) if i not in self.extremes]
        ...
        for ii in range(n):
            ...
            for jj in range(n):
                ...
                if jj==ii:
                    self.M[ii,jj] = mu0 * ( matrix_diagonal(h[0],ri) + matrix_diagonal(h[1],ri) )
                ...
                # Handle singularity in neighboring dofs
                    ...
                    for k in range(2):
                        if j_neighbors[k] == dof_i:
                            ...
                            h_far = h[k]
                    self.M[ii,jj] = mu0 * (matrix_close(h_close,ri) + matrix_far(h_far,Gj))

                # Simple trapezoidal quadrature far from singularity
                else:
                    self.M[ii,jj] = mu0 * (matrix_far(h[0],Gj) + matrix_far(h[1],Gj))
                ...
                # Handle singularity in neighboring dofs
                    ...
                    if j in self.extremes:
                        ...
                        self.M[i,j] = mu0 * (matrix_close(h_close,ri) + matrix_far(h_far,Gj))
    ...
    def integral_mask(self,m2d):
        ...
        @param m2d: mesh of the 2D domain.
        ...
        @TODO: This implementation assumes a rectangular domain.
        This might not be valid in general cases, and should be generalized.
        '''
    ...
    def solve_boundary_integral_eq(self,psi):
        '''
        Solves the boundary integral equation for V(q),
        solves the linear system to compute q from V(q).

        @param psi: flux function
        @return q: Neumann boundary datum
        '''
        ...
        vq = Function(self.Q).assign(0.5 * psi_trace + self.K_func)
        '''
        p = TestFunction(self.Q)
        v = TrialFunction(self.Q)
        a = v * p * dx(domain=self.m)
        L = (0.5 * psi_trace + self.K_func) * p * dx(domain=self.m)
        bcs = [DirichletBC(self.Q,0.0,"on_boundary")]
        solve(a==L,vq,bcs=bcs)
        '''
        # Solve the linear system to obtain g from V(q):
        #return np.linalg.solve(self.M, vq.dat.data_ro[:])
        no_extremes = [i for i in range(len(self.Q_dofs)) if i not in self.extremes]
        vq_vec = vq.dat.data_ro[no_extremes]
        q = Function(psi.function_space())
        q.dat.data[self.Q_dofs[no_extremes]] = np.linalg.solve(self.M,vq_vec) 
        return q


    def compute_datum(self,psi):
        '''
        Computes the neumann boundary datum g using bundary element method.

        @param psi: flux function
        '''

        # Compute and update K(psi):
        self.compute_K(psi)

        # Compute Neumann boundary datum:
        q = self.solve_boundary_integral_eq(psi)
        
        # TEST FOR g -> check compatibility condition
        '''
        mesh = V.mesh()
